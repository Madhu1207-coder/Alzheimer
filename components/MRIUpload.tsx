...
export default function MRIUpload({ initialFile, onResult }: MRIUploadProps) {
  const { t } = useLanguage()
  ...
  useEffect(() => {
    const autoAnalyzeSample = async () => {
      try {
        ...
        const sampleImages = getRandomMRIImages(undefined, 1)
    ...
    if (initialFile && !uploadedFile && !result && !processing) {
      handleFile(initialFile)
      return
    }

    // Only auto-analyze if no file is provided/uploaded and no result exists
    if (!initialFile && !uploadedFile && !result && !processing) {
      autoAnalyzeSample()
    }
  }, [initialFile]) // Add initialFile to dependencies
      ...
      {processing && (
        <div className="bg-blue-50 border border-blue-200 rounded-lg p-4 sm:p-6">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 space-y-2 sm:space-y-0">
            <div className="flex items-center">
              <div className=" rounded-full h-5 w-5 sm:h-6 sm:w-6 border-b-2 border-blue-500 mr-2 sm:mr-3"></div>
              <span className="text-blue-700 font-medium text-sm sm:text-base">{t('mri.upload.processingStatus')}</span>
            </div>
      ...
      {result && (
        <div className="space-y-4 sm:space-y-6">
          ...
          <div className={`rounded-lg border p-4 sm:p-6 ${getConfidenceBg(result.prediction.confidence)}`}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-3 sm:mb-4 space-y-2 sm:space-y-0">
              <h3 className="text-base sm:text-lg font-semibold text-gray-900">{t('mri.upload.predictionResults')}</h3>
              <div className="flex items-center">
                <CheckCircle className="h-4 w-4 sm:h-5 sm:w-5 text-green-500 mr-1 sm:mr-2" />
                <span className="text-xs sm:text-sm text-gray-600">
                  {t('mri.upload.processedIn')} {(result.processingTime / 1000).toFixed(1)}s
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6">
              <div>
                <h4 className="font-medium text-gray-800 mb-2 sm:mb-3 text-sm sm:text-base">{t('mri.upload.classification')}</h4>
                <div className="space-y-2">
                  ...
                  <div className="text-xs sm:text-sm text-gray-600">
                    {t('mri.upload.modelConfidence')}
                  </div>
                </div>
              </div>

              <div>
                <h4 className="font-medium text-gray-800 mb-2 sm:mb-3 text-sm sm:text-base">{t('mri.upload.classProbabilities')}</h4>
                <div className="space-y-2">
