...
export function getRandomMRIImages(category?: keyof typeof MRI_CATEGORIES, count: number = 5): MRIImage[] {
  // Shuffle categories if not specified to ensure randomness
  const shuffledCategories = category ? [category] : (Object.keys(MRI_CATEGORIES) as (keyof typeof MRI_CATEGORIES)[]).sort(() => Math.random() - 0.5)
  ...
  shuffledCategories.forEach(cat => {
    const categoryImages = [...(realImages[cat] || [])].sort(() => Math.random() - 0.5)
    // ...
