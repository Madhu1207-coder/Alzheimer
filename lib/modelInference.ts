...
function simulateMRIPrediction(imageFile: File) {
  ...
  if (fileName.includes('normal') || fileName.includes('healthy') || fileName.includes('nondem')) {
    baseClass = 'Normal'
    baseConfidence = 0.92
  } else if (fileName.includes('verymild') || fileName.includes('mci')) {
    baseClass = 'MCI'
    baseConfidence = 0.76
  } else if (fileName.includes('moderate')) {
    baseClass = 'Moderate AD'
    baseConfidence = 0.88
  } else if (fileName.includes('mild') || fileName.includes('ad')) {
    baseClass = 'Mild AD'
    baseConfidence = 0.82
  } else {
    // Uniform random prediction for unknown files
    const rand = Math.random()
    if (rand < 0.25) baseClass = 'Normal'
    else if (rand < 0.50) baseClass = 'MCI'
    else if (rand < 0.75) baseClass = 'Mild AD'
